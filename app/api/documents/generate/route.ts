...
export async function POST(req:NextRequest){
 ...
 const data=await getDocumentData(type,id); const discountPercent=Math.max(0,Number(body.discountPercent)||0); const taxRate=Math.max(0,Number(body.taxRate)||0); data.item=data.item.map(x=>({...x,discount:discountPercent,taxRate})); data.subtotal=data.item.reduce((s,x)=>s+x.quantity*x.rate,0); data.discount=data.item.reduce((s,x)=>s+x.quantity*x.rate*(x.discount/100),0); data.tax=(data.subtotal-data.discount)*(taxRate/100); data.total=data.subtotal-data.discount+data.tax; if(body.paymentTerms) data.paymentTerms=String(body.paymentTerms); if(body.deliveryTerms) data.deliveryTerms=String(body.deliveryTerms); const now=new Date(); const year=now.getFullYear(); const prefix=({PURCHASE_ORDER:"PO",SALES_ORDER:"SO",INVOICE:"INV",QUOTATION:"QUO"} as any)[type];
