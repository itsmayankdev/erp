...
export async function POST(req:NextRequest){
 ...
 const data=await getDocumentData(type,id); const discountPercent=Math.max(0,Number(body.discountPercent)||0); const gstRate=Math.max(0,Number(body.gstRate)||0); data.item=data.item.map(x=>({...x,discount:discountPercent,gstRate})); data.gstRate=gstRate; data.subtotal=data.item.reduce((s,x)=>s+x.quantity*x.rate,0); data.discount=data.item.reduce((s,x)=>s+x.quantity*x.rate*(x.discount/100),0); data.tax=(data.subtotal-data.discount)*(gstRate/100); data.total=data.subtotal-data.discount+data.tax; if(body.paymentTerms) data.paymentTerms=String(body.paymentTerms); if(body.deliveryTerms) data.deliveryTerms=String(body.deliveryTerms); const now=new Date(); const year=now.getFullYear(); const prefix=({PURCHASE_ORDER:data.company.purchaseOrderPrefix,SALES_ORDER:data.company.salesOrderPrefix,INVOICE:data.company.invoicePrefix,QUOTATION:data.company.quotationPrefix} as any)[type];
 ...
 return NextResponse.json({ok:true,url:"/api/documents/"+row.id+"/file",filename,type,number:docNumber,version:row.version});
