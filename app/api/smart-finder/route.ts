...
export async function GET(req:NextRequest){
  ...
  for(const s of stocks){
    ...
    if(location)score+=5;
    results.push({id:s.id,stockId:s.id, source:"Company inventory",material:s.material.name,grade:s.material.grade,specification:s.material.specification,quantity:qty,unit:s.material.unit,rate,location:s.warehouse?.city||s.warehouse?.name,matchScore:Math.min(score,99),seller:"Steel OS Inventory"});
  }
