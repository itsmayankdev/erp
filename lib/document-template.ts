...
export type DocumentData = { type:DocumentType; number:string; date:Date; company:any; party:any; item:any[]; subtotal:number; discount:number; tax:number; gstRate?:number; total:number; paymentTerms?:string; deliveryTerms?:string; notes?:string; terms?:string };
...
export function buildSvg(d:DocumentData){
  ...
  const startY=315;
  const totalsY=Math.min(Math.max(startY+d.item.length*38+35,520),735);
  return `<svg xmlns="http://www.w3.org/2000/svg" width="794" height="1123" viewBox="0 0 794 1123">
    ...
    ${text(500,totalsY+44,"GST @ "+(Number(d.gstRate)||0)+"%",10,"400","#64748b")}
    ...
    ${text(55,totalsY+37,String(d.terms||d.notes||"").slice(0,95),8,"400","#475569")}
...
export async function buildPdf(d:DocumentData){
 ...
 y-=55; t("TERMS & NOTES",40,y,8,bold,muted); y-=15; wrap(safe(d.terms||d.notes),78).forEach((line,i)=>t(line,40,y-i*10,7,font,muted)); t("Authorized Signatory",40,90,8,font,muted); page.drawLine({start:{x:40,y:105},end:{x:190,y:105},thickness:.7,color:muted}); t("Generated by Steel Trading ERP",40,45,6,font,muted);
 return pdf.save();
}