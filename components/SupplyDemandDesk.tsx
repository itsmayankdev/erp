   ...
   <div className="sdForm">
    ...
    <div className="sdSection"><div className="sdSectionTitle"><b>Material details</b><span>Existing master data auto-fills the known specification.</span></div>
      <div className="sdGrid">
       ...
       <label>Grade<input type="text" name="record-grade" autoComplete="off" spellCheck={false} value={newMaterial?materialGrade:gradeValue} onChange={e=>{const value=e.currentTarget.value; if(newMaterial)setMaterialGrade(value); else setGradeValue(value)}} onInput={e=>{const value=e.currentTarget.value; if(newMaterial)setMaterialGrade(value); else setGradeValue(value)}} onFocus={e=>e.currentTarget.select()} placeholder={newMaterial?"e.g. IS 513":"Auto-filled — click and edit"} style={{pointerEvents:"auto",cursor:"text"}} /></label>
       <label>Specification<input type="text" name="record-specification" autoComplete="off" spellCheck={false} value={newMaterial?materialSpec:specValue} onChange={e=>{const value=e.currentTarget.value; if(newMaterial)setMaterialSpec(value); else setSpecValue(value)}} onInput={e=>{const value=e.currentTarget.value; if(newMaterial)setMaterialSpec(value); else setSpecValue(value)}} onFocus={e=>e.currentTarget.select()} placeholder={newMaterial?"e.g. 0.8mm x 1250mm":"Auto-filled — click and edit"} style={{pointerEvents:"auto",cursor:"text"}} /></label>
    ...
    <div className="sdSection"><div className="sdSectionTitle"><b>{tab==="supply"?"Availability & commercial details":"Requirement & commercial details"}</b></div>
      <div className="sdGrid">
       ...
       <label>Location<input value={form.location} onChange={e=>setForm({...form,location:e.target.value})} placeholder="Factory / delivery location"/></label>{tab==="supply"&&<label>Supply status<select value={form.status} onChange={e=>setForm({...form,status:e.target.value})}><option>Open</option><option>Urgent</option><option>Hot</option><option>Partially Allocated</option><option>Converted</option><option>Closed</option></select></label>}
